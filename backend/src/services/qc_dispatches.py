...
from models.production import ProductionItem, ProductionMovement, WorkOrder, WorkOrderBatch
from services.errors import DomainError
from services.procedure_tags import is_final_tag_set
    ...
    if dispatched is None:
        dispatched = session.scalar(
            select(func.coalesce(func.sum(ProductionMovement.quantity), 0)).where(
                ProductionMovement.work_order_batch_id == batch.id,
