...
export function useProductEditorForm() {
  const form = reactive<ProductForm>({
    ...
    factory_code: '',
    customer_code: '',
    bom_items: [{ part_name: '', part_no: '', pcs: '1 pcs', remark: '' }],
  ...
  function validateBom(): string | null {
    ...
    for (const [index, item] of form.bom_items.entries()) {
      if (!item.part_name.trim() || !item.part_no.trim() || !item.pcs.trim()) {
        return `BOM 第 ${index + 1} 行的配件名称、图纸编号和用量必须填写`
      }
      const partNo = item.part_no.trim()
      if (partNumbers.has(partNo)) return `BOM 图纸编号重复：${partNo}`
