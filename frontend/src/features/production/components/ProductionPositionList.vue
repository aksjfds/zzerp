...
import type { StandardWorkbenchPosition } from '../domain/productionWorkbench'

defineProps<{
  items: StandardWorkbenchPosition[]
  loading: boolean
  selectedKey?: string
}>()
const emit = defineEmits<{ select: [item: StandardWorkbenchPosition] }>()
</script>
...
<template>
  <div v-loading="loading" class="position-list">
    ...
    <ElEmpty v-if="!loading && !items.length" description="当前部门暂无在位物料" :image-size="72" />
  </div>
</template>
